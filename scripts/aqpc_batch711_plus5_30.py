"""Enter 5 more AQPC payment-request bills on batch 711 (30-row sheet).

Skips KIMCO 10007–10031 and Kyle-entered 10917/10918/10920/10921.
...
LOGGER = logging.getLogger("ap_clerk.aqpc_batch711_30")
...
# 10951 / 10949–10947 not in mailbox last time). Discovery fills extras.
PREFERRED_FIVE = ["10933", "10932", "10929", "10928", "10927"]
KNOWN_TWENTY_FIVE = [
...
PRIOR_SHEET = ROOT / "runs" / "AP-run-2026-09-15-aqpc-batch711-25.json"
HOLD_PDF_AMOUNTS = {
    10009: 10.0,
    10013: 199.0,
    10021: 700.0,
...
def find_aqpc_payment_requests(graph) -> list[dict[str, Any]]:
    ...
    extras = [
        ...
        for n in [
            ...
            *[str(n) for n in range(10890, 10934)],
        ]
        if n not in found
    ]
    for n in extras:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Enter 5 more AQPC invoices on batch 711 (30-row sheet)")
    parser.add_argument("--discover-only", action="store_true")
    parser.add_argument("--skip-enter", action="store_true")
    parser.add_argument(
        "--report",
        default=str(ROOT / "runs" / "AP-run-2026-09-15-aqpc-batch711-30.xlsx"),
    )
    ...
    print(f"ALL 30: Success={all_success} HOLD={all_hold}", flush=True)
    return 0
