...
Dedicated live batch — do NOT reuse Crosslink 715 or JPSteel 716.
...
9995 / 9996), and signed packing slips / receipt scans. Prefer invoice
dates on/after 2026-08-01. If fewer than 5 recent remain, enter what's
left and stop — do not walk pre-Aug (NOTE-28).
"""
...
def already_set(entered: dict[str, int]) -> set[str]:
    out: set[str] = set()
    for number in list(entered) + list(KNOWN_ENTERED):
...
def main(argv: list[str] | None = None) -> int:
    ...
    recent, older, slips = pick_recent(parsed_bills, already=already, cap=CAP)
    print(
        json.dumps(
            {
    ...
    if not recent:
        ...
        rows = leftover_slip_rows(slips, batch_label)
    ...
    sidecar = {
        ...
        "new_rows": new_rows,
        "chosen_because": (
            "First pass on a new 9/17 Legacy Wire batch. Unflagged Legacy Wire "
