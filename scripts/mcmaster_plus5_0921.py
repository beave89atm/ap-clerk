...
Do not create a new batch. Do not mutate headers 10138–10147.
...
LOGGER = logging.getLogger("ap_clerk.mcmaster_plus5_0921")
SHEET_INVOICES = set(CREATED_HEADERS) | set(PLUS5_HEADERS) | set(PLUS10_HEADERS)
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        "Target: live. McMaster plus-5 on existing batch 721. "
        "Leave 10138-10147 alone. No Mail.Send. invent=false.",
        flush=True,
    )
    if not creds.ready:
    ...
    PLUS10_HEADERS.update({k: int(v) for k, v in created.items() if v not in (None, "")})
    merged = merge_sheet_rows(prior_rows, rows)
    write_report(report_path, merged)
    sidecar = {
        "proof": "mcmaster-plus5-0921",
        "invent": False,
        "mail_send": False,
        "vendor": VENDOR_NAME,
        "vendor_id": vendor_id,
        ...
        "preferred_plus10": list(PREFERRED_PLUS10),
