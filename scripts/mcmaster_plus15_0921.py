"""Add 5 more McMaster-Carr invoices to existing batch 721.

Do not create a new batch. Do not mutate headers 10138–10152.
...
LOGGER = logging.getLogger("ap_clerk.mcmaster_plus15_0921")
SHEET_INVOICES = set(CREATED_HEADERS) | set(PLUS5_HEADERS) | set(PLUS10_HEADERS) | set(PLUS15_HEADERS)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Enter 5 more McMaster bills on existing batch 721")
    ...
    print(
        "Target: live. McMaster plus-15 on existing batch 721. "
        "Leave 10138-10152 alone (10142 Kyle; Transfer AP 10140/10143/10148 Treyce). "
    ...
    collision = set(PREFERRED_PLUS15) & (set(entered) | SHEET_INVOICES | already_on_sheet)
    if collision:
        print(f"Preferred plus-15 already on KIMCO/sheet: {sorted(collision)}. Stop.", flush=True)
        return 2
        ...
        json.dumps(
            {
                ...
                "preferred_plus15": list(PREFERRED_PLUS15),
            },
            indent=2,
            default=str,
        ),
    ...
    PLUS15_HEADERS.update({k: int(v) for k, v in created.items() if v not in (None, "")})
    merged = merge_sheet_rows(prior_rows, rows)
    write_report(report_path, merged)
    sidecar = {
        "proof": "mcmaster-plus15-0921",
        "invent": False,
        "mail_send": False,
        "vendor": VENDOR_NAME,
        "vendor_id": vendor_id,
        ...
        "preferred_plus15": list(PREFERRED_PLUS15),
        "plus15_headers": dict(PLUS15_HEADERS),
