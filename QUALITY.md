...
6. **PPV only** for unit-price gaps vs PO, and only if ≤10% of invoice total **and** ≤$100; else price-does-not-match HOLD + `@Shawn McKibben`.
   ...
   **Kyle + Shawn 2026-09-21 (NOTE-46):** when PO UOM/pack differs from
   ...
   gap is only cents / still within the PPV gate, **Select Receipts +
   signed PPV + Success**. Do **not** HOLD `price_variance` and do **not**
   Transfer AP for that class. Over-gate still NOTE-29 / NOTE-40.
...
| **NOTE-46** PO UOM/pack vs invoice in-gate PPV is Success (Kyle + Shawn 2026-09-21 McMaster **71080498 / 10139**) | Ordered 72 inches vs invoice 2×3 foot bars; price off by a few cents. Treating that as `price_variance` HOLD / Transfer AP is wrong — Shawn: "This should fall under PPV". | When PO UOM/pack differs from the invoice and the $ gap is cents / still within the PPV gate (≤10% of invoice total **and** bill PPV ≤$100): **Select the matching receipts**, post signed **Purchase Price Variance** (lookup id 13) so Invoice_Amount hits the PDF, **Success**, stay on the current API Agent batch. Do **not** HOLD `price_variance`. Do **not** Transfer AP. Fees/shipping stay Additional Charge Fees id 11 (NOTE-44). Over-gate still NOTE-29 / NOTE-40 (no Select those lines). | `test_never_repeat_uom_pack_in_gate_ppv` |
