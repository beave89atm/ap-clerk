...
- After a packing-slip attach is **GET-verified** on the matching AP
  header, set that receiving@ Outlook message category to exactly
  **`AI Completed`**. Only after verify. Do not stamp unmatched /
  skipped / failed attaches. One email → many invoices: stamp only when
