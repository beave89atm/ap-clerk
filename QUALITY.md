...
vs invoice line is outside the PPV gate (|bill PPV| **$75 or more**,
NOTE-47), **do not Select Receipts** for that line (selecting
...
5. **Fees/surcharges → Fees and surcharges** — never PPV (Techni-Tool $46.20).
   ...
   **Kyle 2026-09-21 (NOTE-44):** shipping/surcharges are **not** missing
   merch receipt lines. Do not HOLD `missing_receipt` when the only
   non-receipt PDF dollars are Fees already posted (or to be posted) as
   Additional Charge Fees id 11. Success still requires Select Receipts
   for merch + Fees for shipping/surcharges + amount = PDF. McMaster
   **71839575 / 10147** (and 10144 / 10145) is the class. **72094446 /
   10142** is real missing merch leftovers — keep HOLD.
6. **PPV only** for unit-price gaps vs PO, and only if **|bill PPV| is
   under $75** (NOTE-47). Else price-does-not-match HOLD + `@Shawn McKibben`.
   The 10%-of-invoice-total rule is **deleted**. The old $100 cap is
   replaced by $75. Negative PPV is allowed; the gate is absolute dollars.
   **Kyle 2026-09-16 (NOTE-29):** if a leftover is over that gate, **do not
   ...
   PDF. AQPC **11003 / 10009** and **10991 / 10013** stay GET-only leftovers
   (released under the old 10% gate).
   ...
   **Kyle + Shawn 2026-09-21 (NOTE-46):** when PO UOM/pack differs from
   the invoice (72 inches ordered vs 2×3 foot bars invoiced) and the $
   gap is still under $75, **Select Receipts + signed PPV + Success**.
   Do **not** HOLD `price_variance` and do **not** Transfer AP for that
   class. Over-gate still NOTE-29 / NOTE-40.
   **Kyle 2026-09-21 (NOTE-47):** measure |total signed PPV on the bill|
   needed to hit PDF after Select Receipts / fees. Under $75 → Success
   on the current batch. $75 or more → no Select those lines; HOLD
   `price_variance`; Transfer AP + Comments_1 @Shawn 104.
...
| **NOTE-15** EMJ Z250725432 / PO 58913 / KIMCO 9969 (Kyle 2026-09-11) | Two invoice lines; `lines:[]` then one PO receipt; Success; skipped line not on sheet; random-length gap not PPV | Parse both lines; Select Receipts per line; unmatched → not Success + Why names the skipped line; small length variance is PPV when \|bill PPV\| is under $75 (NOTE-47), not Fees; prepaid/ship-date null amount is not a fee | `test_never_repeat_emj_z250725432_two_lines` |
...
| **NOTE-29** Over-PPV leftover must not be selected (AQPC **11003 / 10009** receipt 24103; **10991 / 10013** receipt 23967) | Headers created; over-PPV leftovers were still Select Receipts’d (~84% / $8.45 on 11003; ~25% / $49.75 on 10991). That locked the receipt so Shawn could not unreceive, fix the PO price, and re-receive. | If leftover vs invoice line is outside the PPV gate (\|bill PPV\| **$75 or more**, NOTE-47), **do not Select Receipts** for that line. Whole bill over-gate → select **zero**. Still header + PDF. HOLD `price-does-not-match` + `@Shawn McKibben`. Outlook Entered with issues. Never Success. The 10% / $100 rules are deleted. Kyle 2026-09-16: release 24103 / 23967 on 10009 / 10013 (GET-only leftovers). | `test_never_repeat_aqpc_over_ppv_does_not_select_receipts` |
...
| **NOTE-44** Additional Charge Fees/surcharges are not missing merch lines (Kyle 2026-09-21 McMaster **71839575 / 10147**) | Sheet kept `HOLD missing_receipt` after merch Select Receipts + Fees id 11 already matched the PDF. Agent treated shipping/surcharge dollars as leftover merch lines. 10147 passed Validate Invoice in live UI. Same false HOLD class as 10144 / 10145. | McMaster (and general): shipping/surcharges live under Additional Charges **Fees** (lookup **id 11**). Do **not** HOLD `missing_receipt` when the only non-receipt PDF dollars are Fees already posted or to be posted as Fees id 11. Success gate still requires Select Receipts for merch + Fees for shipping/surcharges + amount = PDF. **Do not change 10142 / 72094446** — that bill still has multiple merch PDF lines without leftovers (real `missing_receipt`). | `test_never_repeat_fees_are_not_missing_receipt` |
| **NOTE-45** Exception action is Comments_1 @tags, not email (Kyle 2026-09-21) | Exception owners were told by Mail.Send / Outlook only. Kyle: move away from emails for exception action. | Default notify path is **Comments_1** mention-node. **No Mail.Send** for Shawn/Ruben exception action unless Kyle explicitly asks. **price_variance / over-PPV** → `@Shawn McKibben` mention-id **104** + move to **Transfer AP** (Treyce-only follow-through; do not Select over-gate lines; do not watch that batch). **missing_receipt McMaster-Carr** → `@Shawn McKibben` (104) on Comments_1; **leave the header on the current API Agent batch** — do **not** Transfer AP. **missing_receipt other vendors** → `@Ruben Perez` once his mention-id is proven from live Comments_1 / user directory (invent=false; 2026-09-21 scan found no Ruben mention-node). | `test_never_repeat_missing_receipt_never_transfer_ap` |
| **NOTE-46** PO UOM/pack vs invoice in-gate PPV is Success (Kyle + Shawn 2026-09-21 McMaster **71080498 / 10139**) | Ordered 72 inches vs invoice 2×3 foot bars; price off by a few cents. Treating that as `price_variance` HOLD / Transfer AP is wrong — Shawn: "This should fall under PPV". | When PO UOM/pack differs from the invoice and the $ gap is still under the $75 PPV gate (NOTE-47): **Select the matching receipts**, post signed **Purchase Price Variance** (lookup id 13) so Invoice_Amount hits the PDF, **Success**, stay on the current API Agent batch. Do **not** HOLD `price_variance`. Do **not** Transfer AP. Fees/shipping stay Additional Charge Fees id 11 (NOTE-44). Over-gate (\|bill PPV\| $75 or more) still NOTE-29 / NOTE-40 (no Select those lines). | `test_never_repeat_uom_pack_in_gate_ppv` |
| **NOTE-47** PPV gate is \|bill PPV\| under $75 (Kyle 2026-09-21) | Old gate was ≤10% of invoice total **and** ≤$100. Small invoices (AQPC 11003 $8.45 / ~84%) were treated as over-gate because of the percent rule. $100 was the dollar cap. | **Delete the 10% rule.** Measure the absolute dollar total of signed Purchase Price Variance Additional Charges needed to hit the PDF after Select Receipts / fees. \|total PPV\| **under $75** → Select matching receipts, post signed PPV, Success on the current batch. \|total PPV\| **$75 or more** → do **not** Select those over-gate lines; HOLD `price_variance`; Transfer AP + Comments_1 @Shawn McKibben mention-id **104**. Negative PPV allowed; gate on absolute dollars. Fees/shipping stay Additional Charge Fees id 11 (never PPV). NOTE-46 UOM/pack still uses PPV (not HOLD) when under $75. | `test_never_repeat_ppv_gate_abs_75` |
...
outside the PPV gate (|bill PPV| **$75 or more**, NOTE-47). The
10%-of-invoice-total rule and the old $100 cap are deleted.
