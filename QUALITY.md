...
| **NOTE-57** Live PPV QC before Success (Kyle 2026-09-25) | Penny PPV was a one-off. Entry could still report Success when verification missed lines + charges, because Invoice_Amount had already rolled to the line total. | Re-read live after create/fix. gap = verification (else Invoice_Amount) − (receipt/misc lines + all charges). \|gap\| < $75 → one signed PPV and re-read. \|gap\| >= $75 → HOLD price_variance, no PPV. Success only when the live gap is 0.00 and the Invoice_Amount rollup gap is 0.00. Spreadsheet columns unchanged; Notes only when a PPV QC charge posted. Before the sheet is written or sent, re-read the run and drop Success on a nonzero gap. Read-only: `python -m ap_clerk.ppv_qc --live --batch <id>`. | `tests/test_ppv_qc.py` |
