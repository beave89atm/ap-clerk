...
**Totals match before Finish (Kyle).** Always make sure totals match before finishing. Header invoice total == PDF total == selected receipt lines + all charges, to the penny. Run this check before the Finish step. If `|gap| < ppv_limit()` ($75 unless `AP_PPV_LIMIT` is set), post one signed Purchase Price Variance first and re-read. If `|gap| >= ppv_limit()`, HOLD `price_variance` and do not post PPV. Finish is blocked in code when that pre-check fails. Run the same check again after Finish on the live readback. Success only when that readback is still 0.00. Do not post the bill. Do not move the batch.
...
**Treyce 2026-09-16 AP-run notes (NOTE-30–36).** Encode as never-repeat
QUALITY gates. Weekday FIFO is paused — code/tests/QUALITY first.
...
categorize-at-creation; Kyle product bar #4 / #6). Excel columns
`Exception category` and `Exception owner` (empty for Success and true
Skipped noise). Workbook **Exception counts** sheet is counts by
category only — never invent Success/touchless rates. Map existing
...
Exception owner. **Misty McCoy is not a hard default.** Comments_1 and
run-sheet Notes are plain English (NOTE-59), not a `category=…; owner=…`
prefix.
...
**NOTE-51 Success finish updates Outlook (Kyle 2026-09-22).** When you **fix an existing KIMCO invoice to Success**, update the matching `accountspayable@` category: `AI HOLD` / `Entered with issues` → **`Entered in AI`**. Do not leave Success bills sitting on AI HOLD. Multi-invoice parent emails flip to `Entered in AI` only when **all** sibling invoices from that PDF are Success; otherwise keep `Entered with issues` / `AI HOLD` as appropriate. `receiving@` packing-slip emails still get **`AI Completed`** after a verified slip attach (separate mailbox / NOTE-49). No Mail.Send.

**NOTE-52 Move entered mail to Inbox `9 - FORT WORT…` (Kyle 2026-09-22).** After a KIMCO AP **header is created** and the **vendor PDF is attached**, move the source `accountspayable@` email into the Inbox child folder whose displayName starts with `9 - FORT WORT` / `9-Fort Worth` (contains `FORT WORT`, case-insensitive). Multi-invoice parent: move **once** when the first header+attach from that message succeeds — do not wait for all siblings Success. Keep existing categories (`Entered in AI` / `Entered with issues` / `AI HOLD`). Do **not** move if header create failed or PDF was not attached. No Mail.Send.

**NOTE-53 missing_receipt → Transfer AP (Kyle 2026-09-22).** When Result = HOLD `missing_receipt` (no selectable matching leftovers / cannot Select Receipts), move the header to **Transfer AP** (batch name lookup; prior fact **375** is a hint only — never invent). Same PUT path as over-PPV. Still write Comments_1 @tag from the vendor receiving-owner map. Do **not** invent Success. Do **not** Select non-matching leftovers. Over-PPV / `price_variance` still Transfer AP + @Shawn (unchanged). AP Clerk does not watch/finish Transfer AP unless Kyle asks. Outlook categories unchanged by this batch move. **Supersedes** the 2026-09-21 rule that left `missing_receipt` on the current API Agent batch.

**NOTE-54 classify-before-extract for invoice+statement mixed PDFs (Kyle 2026-09-24 / Gas 0040406308 class).** NOTE-24 skips a whole Account Statement email. A **mixed PDF** (invoice pages and statement pages in one attachment) is different: classify each page **before** header create, amount parse, or Success. Classes: `invoice` | `statement` | `other` (Account Statement, Statement of Account, past-due list, aging, remittance advice versus Invoice / Amount Due / invoice #). If any invoice pages **and** any statement pages: enter, attach, and total **invoice pages only** (page-range PDF when the slice is feasible; otherwise the full pack plus Why naming the ignored statement pages). Invoice total, lines, and invoice # come from invoice pages only — never the statement balance. Statement-only PDF stays NOTE-24 `Skipped` + Outlook `AI Skipped 2`. Same-page or otherwise uncertain boundaries → HOLD `pdf_capture` (Entered with issues only if a header already exists) with Why naming the page classes. Never Success.

**NOTE-55 open receipts forbid `missing_receipt` (Kyle 2026-09-24 QC).** When open/selectable leftovers exist on the PO path and qty/cost does not match, the exception category is `quantity_variance`. When matching lines are already billed, the category is `already_entered`. **Never** `missing_receipt` in those cases — that label sends Transfer AP to the wrong owner. Comments_1 / Why states the exact ask (receive qty X of part Y on PO Z, or qty mismatch invoice N vs receipt M), not a vague "missing receipt". The receiving-owner @tag still applies only for a true `missing_receipt` (no selectable leftovers).

**NOTE-56 penny PPV when lines + charges miss the header (Kyle 2026-09-25 / Gas 0040443847 / KIMCO 10284).** `parse_money` stores a 4-decimal unit as cents (PDF 1.2076 → Unit_Price 1.21). KIMCO then extends qty × that price (12 × 1.21 = 14.52) instead of the printed extension 14.49. The 2026-09-23 gap enter (`gap_four_vendors_0923` → `apply_type4_misc_lines`) left verification at the PDF total $29.25 and Invoice_Amount at $29.28, with no Purchase Price Variance. Type 4 no-PO finish returned `ppv_status=none`. Per-line `decide_ppv` still treats a two-cent line compare as a match; that waiver does **not** apply once stored lines + charges differ from the header. Post one signed Additional Charge **Purchase Price Variance** (lookup id 13) for the exact remainder, including $0.01, $0.02, and $0.03. |gap| >= $75 is not a penny PPV. Do not change the batch, receipts, or verification, and do not post the bill. Success only when lines + charges equal the header to the penny. 10284 → PPV **−$0.03**. The same enter left 0040446744 / KIMCO **10283** at extensions **$1,891.85** against verification **$1,891.88**. That gap is **+$0.03**. A negative PPV would leave lines + charges at $1,891.82.

**NOTE-57 live PPV QC before Finish and again after Finish (Kyle 2026-09-25).** Before the Finish step, and again after Finish on the live readback, re-read the bill. Header invoice total (`Invoice_Amount`), PDF total (`Invoice_Verification_Amount`), and selected receipt lines + all charges must match to the penny. Finish is blocked in code when the pre-check fails. After every bill is created or fixed, re-read it live. `gap` = invoice header total − (selected receipt lines and Type 4 misc lines + every additional charge, including fees, freight, and PPV). The header total is `Invoice_Verification_Amount` when that field is set, because that is the PDF total and it does not move when a charge posts. `Invoice_Amount` is the rollup of lines + charges, so `Invoice_Amount` − (lines + charges) was already 0.00 on 10284 and 10283 while verification was still off by a few cents. Success also requires that rollup gap to be 0.00. If the header gap is not 0.00 and `|gap| < $75`, post one signed Purchase Price Variance (lookup id 13) for the exact gap and re-read. If `|gap| >= $75`, HOLD `price_variance` and do not post PPV. No merchandise lines does not invent a full-invoice PPV. A bill may not be reported Success unless the live readback gap is 0.00. Before a run spreadsheet is written or emailed, re-read every bill in the run and change Success to HOLD when the gap is not 0.00. The spreadsheet columns do not change. The existing Notes column is filled only when this gate posted a PPV. The gate does not post the bill and does not move the batch. Read-only scan: `python -m ap_clerk.ppv_qc --live --batch 720 --batch 722 --batch 724 --batch 727 --batch 730`. There is no tolerance: a live gap of **$0.01** posts a PPV. O'Neal **15464074** / KIMCO **10318** / receipt **23880** was PDF **$192.85** versus receipt extended **$192.86**. `decide_ppv` had treated that one cent as a match (`abs(variance) <= 0.02`). One cent is now a signed PPV (−$0.01 on 10318). Exactly **$0.02** stays a match for NOTE-23. The absolute PPV limit is one setting, `ppv_limit()` / `PENNY_PPV_MAX_ABS` (**$75**), overridable with `AP_PPV_LIMIT`, and each run workbook logs it on the AP run sheet header and the Exception counts sheet.

**NOTE-58 quantity/UOM disconnect over the PPV limit → Transfer AP + @Shawn (Kyle, O'Neal 15455478 / KIMCO 10317).** When a quantity or unit-of-measure disconnect (receipt qty ≠ invoice qty ≠ PO qty, or inches received versus invoiced) produces a dollar gap at or over `ppv_limit()` (default **$75**), the bill is **HOLD** `quantity_variance`. Move it to **Transfer AP** with the same name lookup used for `missing_receipt` (prior fact 375 is a hint only — never invent). Write one **Comments_1** note that @tags **Shawn McKibben** (mention id **104**). Shawn is the owner who unreceives and fixes these, not `buyer`. Do not write the gap off as a price variance. Do not select the mismatched receipt. Do not post the bill. A quantity mismatch whose dollar gap is under the limit stays `quantity_variance` / buyer and does not take this Transfer AP path.

**NOTE-59 plain-English Comments_1 and run-sheet Notes (Kyle).** Every Comments_1 note and every run-sheet Note is plain English: complete, actionable sentences that say what does not match, the actual numbers (invoice total, amount entered, line, PO, receipt, quantity received versus invoiced versus ordered, and the gap), why it cannot be entered as-is, exactly what the owner must do, and what happens after that fix. No `category=…; owner=…` key/value shorthand. No HTML in sheet notes. Keep the `AP Clerk:` prefix. The same shape covers every HOLD reason (`missing_receipt`, `quantity_variance`, `price_variance`, `missing_po`, `vendor_mismatch`, `already_entered`, `pdf_capture`, `auto_pay`, `partial_match`, `other`). Mention HTML belongs only on the KIMCO Comments_1 value, using the existing @Shawn span (user id 104).
...
| **NOTE-39** Exception category + owner at HOLD (Stampli-style; Kyle bar #4 / #6; Kyle 2026-09-18 Shawn owns Purchasing) | HOLD / Incomplete Why was unstructured; sheet could not sort by cause; exception rate by category unmeasurable. `missing_po` / Transfer AP contact was wrongly hard-defaulted to Misty McCoy. | Every HOLD / Incomplete / Entered-with-issues row stamps `Exception category` + `Exception owner`. Success / true Skipped noise leave columns blank. Counts-only summary sheet. Map existing gates only (`price_variance`→Shawn McKibben, `missing_receipt`→Ruben Perez, `quantity_variance`→buyer, `missing_po`→Shawn McKibben, `vendor_mismatch`→AP / vendor master, `already_entered`→none / review, `pdf_capture`→AP, `auto_pay`→none, `partial_match`→AP / Treyce, `other`→AP). Shawn oversees Purchasing (price variance, PO not on live, missing/bad PO). Transfer AP is a destination batch only. Misty McCoy is not a hard default. Never invent Success/touchless rates. Sheet notes do not embed `category=…; owner=…` (NOTE-59). | `test_never_repeat_note39_exception_category_owner` |
| **NOTE-58** O'Neal **15455478** / KIMCO **10317** / PO **59059** receipt **23563** (Kyle) | Receipt 23563 was received as 14,400 IN while the invoice and the PO are 1,200 IN. The dollar gap is over $75, so it is not a price-variance writeoff. The old note named the owner as buyer. | Qty or unit-of-measure disconnect with \|gap\| >= `ppv_limit()` ($75) → HOLD `quantity_variance`, Transfer AP (same move as missing_receipt), Comments_1 @Shawn mention id 104. Shawn owns the fix, not buyer. Do not select the bad receipt. Do not post. Under the limit, quantity_variance stays with the buyer. | `tests/test_qty_uom_shawn.py` |
| **NOTE-59** Plain-English Comments_1 and run-sheet Notes (Kyle) | HOLD notes used `category=…; owner=…` shorthand and were not actionable. | Every Comments_1 note and every run-sheet Note is plain English with the `AP Clerk:` prefix: what does not match, the numbers, why it cannot be entered, what the owner must do, and what happens after. No key/value shorthand. No HTML in sheet notes. All HOLD reasons use that builder. | `tests/test_qty_uom_shawn.py` |
...
| **NOTE-51** Success finish updates AP Outlook (Kyle 2026-09-22) | Finishing an existing header to Success left `accountspayable@` on `AI HOLD` / `Entered with issues` (`stamp_outlook=False` on later Gas finishes). | When a bill is repaired to Success, PATCH the matching AP message to **`Entered in AI`**. Multi-invoice parent: `Entered in AI` only if every sibling from that PDF is Success; else keep `Entered with issues` / `AI HOLD`. `receiving@` still uses **`AI Completed`** after verified slip attach. No Mail.Send. | `tests/test_outlook_finish.py` |
| **NOTE-52** Move entered mail to Inbox `9 - FORT WORT…` (Kyle 2026-09-22) | Entered AP emails stayed in Inbox after header + vendor PDF attach. | After header create **and** PDF attach, Graph-move the source `accountspayable@` message into Inbox child `9 - FORT WORT…` / Fort Worth. Multi-invoice parent: one move on first header+attach. Categories unchanged. No move without header or attach. No Mail.Send. | `tests/test_fort_worth_move.py` |
| **NOTE-53** missing_receipt HOLD → Transfer AP (Kyle 2026-09-22) | 2026-09-21 left `missing_receipt` on the current API Agent batch so dock-wait bills sat in 720/722. | HOLD `missing_receipt` → PUT `AP_Invoice_Batch` to **Transfer AP** (lookup by name; 375 hint only). Same move path as over-PPV. Comments_1 @tag still follows the receiving-owner map. No invent Success / no Select non-matching leftovers. Over-PPV still Transfer AP + @Shawn. Outlook categories unchanged. Supersedes stay-on-current-batch. | `tests/test_missing_receipt_transfer_ap.py` |
| **NOTE-54** Gas 0040406308-class mixed invoice + statement PDF (Kyle 2026-09-24) | NOTE-24 skips a whole Account Statement. A mixed PDF was extracted as one payable, so a statement balance could become the invoice total. | Classify each page before extract: `invoice` / `statement` / `other`. Invoice+statement → invoice pages only (page-range attach when feasible, else full pack + Why naming ignored statement pages). Totals/lines/# from invoice pages only. Statement-only → NOTE-24 Skipped + `AI Skipped 2`. Uncertain boundaries → HOLD `pdf_capture` with page classes in Why. Never Success on a statement total. | `test_never_repeat_note54_invoice_statement_mixed_pdf` |
| **NOTE-55** Open receipts forbid `missing_receipt` (Kyle 2026-09-24 QC) | HOLD labeled `missing_receipt` when leftovers were open and the real issue was qty mismatch or already-billed. Wrong label routes Transfer AP to the wrong owner. | Open leftovers + qty/cost mismatch → `quantity_variance`. Already billed on matching lines → `already_entered`. Never `missing_receipt` in those cases. Why / Comments_1 states the exact ask (receive qty X of part Y on PO Z, or qty mismatch invoice N vs receipt M). Receiving-owner @tag only for true `missing_receipt`. | `test_never_repeat_note55_open_receipts_not_missing_receipt` |
| **NOTE-56** Gas **0040443847** / KIMCO **10284** and **0040446744** / **10283** (Kyle 2026-09-25) | 10284 lines extended to $29.28 vs verification $29.25. 10283 lines extended to $1,891.85 vs verification $1,891.88. No PPV. Type 4 no-PO finish skipped PPV. | When lines + charges ≠ header verification to the penny, post one signed PPV (lookup id 13) for the exact gap, including $0.01 / $0.02 / $0.03. Sign is verification minus lines minus charges. Do not waive it as two-cent rounding. \|gap\| >= $75 is not penny PPV. 10284 → **−$0.03**. 10283 → **+$0.03**. Success only after lines + charges equal the header. | `tests/test_penny_ppv.py` |
| **NOTE-57** Live PPV QC before Finish and again after Finish (Kyle 2026-09-25) | Penny PPV was a one-off. Entry could still report Success when verification missed lines + charges, because Invoice_Amount had already rolled to the line total. Finish could run before the totals were checked. | Before Finish: header invoice total == PDF total == selected receipt lines + all charges, to the penny. \|gap\| < $75 → one signed PPV first, then re-read. \|gap\| >= $75 → HOLD price_variance, no PPV. Finish is blocked in code when that pre-check fails. Run the same check again after Finish on the live readback. Success only when that readback gap is 0.00 and the Invoice_Amount rollup gap is 0.00. Spreadsheet columns unchanged; Notes only when a PPV QC charge posted. Before the sheet is written or sent, re-read the run and drop Success on a nonzero gap. Read-only: `python -m ap_clerk.ppv_qc --live --batch <id>`. | `tests/test_ppv_qc.py` |
