...
| --- | --- | --- |
| **Success** | Finished bill: header + (Select Receipts when PO) + vendor invoice PDF **and** ≥1 signed packing slip from receiving@ (NOTE-49) + self-check (invoice # from PDF, PO not Type 4, right receipt line, qty match, Fees ≠ PPV, PPV in rule, posted vendor matches parsed). Invoice PDF alone is not Success. | `Entered in AI` only |
...
| Result | Meaning | Outlook |
| --- | --- | --- |
| **Success** | Header created **and** (if PO: Select Receipts done) **and** vendor invoice PDF attached **and** ≥1 matched signed packing slip from receiving@ on the same header (NOTE-49). | `Entered in AI` only |
...
3. **Receipt** — before HOLD-no-receipts, second pass: slip # = invoice #, part, qty, PO line, then open receipts on that PO **verified by invoice qty and merchandise cost**. Prefer exact qty (35 vs 36 → 35). Never first-open / `second-open-on-po` Success when multiple open receipts differ. Match part/PO-WO lines (Modern Heat), not first leftover qty. Parse **every** merchandise line from the PDF (EMJ mill descriptions, not empty `lines:[]`). Select Receipts for **each** invoice line; unmatched lines are named on Why and never silent Success. A Gas billing PDF with N invoice numbers becomes **N bill rows** (after-tax Amount Due per invoice); the email still counts as **1** toward the 10-email cap. Already-entered vendor+# is HOLD `already-entered` before a misleading parse slogan. On `missing_receipt`, Comments_1 **@tags the sheet Receiving owner** (NOTE-50); blank owner (AQPC, Gas and Supply) = no dock @tag. Regressions: Capital `26167`; Fastenal `TXFT499356`; Fastenal `TXFT4100079` / KIMCO **9968**; EMJ `Z250725432` / KIMCO **9969**; Gas `0040370068` pack / KIMCO **9970**; Insight `1809`.
4. **Finish** — `Success` requires header + (Select Receipts when PO) + vendor invoice PDF attached + **≥1 matched signed packing slip from receiving@** (NOTE-49; a slip may be more than one page — keep consecutive pages together; multi-slip scan PDFs are not 1 invoice) + **Additional Charge Fees posted when fees were parsed**. Invoice PDF alone is HOLD `missing-packing-slip`, not Success. After a packing-slip attach is GET-verified on the header, receiving@ is categorized exactly **`AI Completed`** — only when every identifiable slip from that email is attached; partial success stays uncategorized. Header-only with blocked-405 attach, receipts not selected, or fees noted on the sheet but not posted = **Incomplete**, not Success, not Entered in AI. Live 2026-09-09: record GET/PUT/PDF attach and Select Receipts (record PUT of `lists.APInvoiceLine`) work API-only. Fees / Freight External / PPV are a record PUT of `lists.InvoiceAdditionalCharges` with `Additional_Charges` lookup `{id, text}` (Fees id 11, Freight External id 1, PPV id 13). Freight companies (Priority 1) use Freight External, not Fees. No-PO bills can finish API-only (header + PDF). Do not open the KIMCO UI for attach.
