...
**Intuit / QuickBooks click-through** (AQPC payment-request links behind
`links.notification.intuit.com`). Unauth GET first; auth wall opens Playwright
with a persisted session. Never commit the session file. CLI prints
present/absent only:

- `AP_CLERK_INTUIT_STORAGE_STATE` — Playwright `storage_state` JSON path (preferred)
- `AP_CLERK_INTUIT_COOKIE_JAR` — optional cookie JSON
- `AP_CLERK_BROWSER_PDF` — `0`/`false`/`no` disables the browser escalate
- `AP_CLERK_BROWSER_PDF_TIMEOUT` — seconds (default 45)

One-time Kyle login (headed, MFA ok): `python -m ap_clerk.browser_pdf --save-session /secure/path/intuit-storage-state.json` then set `AP_CLERK_INTUIT_STORAGE_STATE` to that path. See QUALITY.md.
