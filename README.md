...
**QUALITY V1.1:** `Success` means a **finished bill**, not a header create. Header-only (blocked-405 attach or Select Receipts not posted) is **Incomplete**. Supervised 10-invoice LIVE dry run continuing after **2026-08-18T13:00:12Z** (2026-09-09 America/Chicago, mailbox-cap / noise = Skipped) is recorded below. This build does **not** re-arm the weekday daily 30. Live 2026-09-09 probe (one existing header) proved record GET/PUT/attach; see below.
...
| **Skipped** | Mailbox noise walked past while filling the bill-attempt cap (not-a-bill, statement, CHECK STOP, payment, POD, duplicate). Noted on the sheet only. | No / `none` — never `AI HOLD` |
...
5. **Bill vs noise** — vendor invoices with a PDF must enter (American Quality Powder Coating). While filling the cap, walk past statements, payments, CHECK STOP, internal mail, PODs, and other not-a-bill noise. **Cap = N bill attempts** (Success + real bill HOLD/Fail/Incomplete), **not** N mailbox messages. Noise does not consume the counter. Note skipped noise on the Excel sheet (`Skipped`, vendor/subject/date/Why). Do **not** stamp Outlook `AI HOLD` on noise.
...
- Still skip not-a-bill / statement / POD / CHECK STOP / payment and **replace** so 30 real bills are attempted when possible. Those skips are Excel `Skipped` rows (Flag in Outlook = No). They do **not** get `AI HOLD`.
...
**Supervised dry run 10 (2026-09-09 America/Chicago, from 2026-08-18 cursor):** continued after the same-day 8/16 dry10 cursor (`2026-08-18T13:00:12Z`, processed_count 456). Did not restart at 7/28. Batch `API Agent - 9/9/26` id **703** (reused; create date still 9/9 CT). 10 real bill attempts; 21 statement/payment/not-a-bill skips walked past and sheet-noted as **Skipped** (Outlook flag = No / `none`, no `AI HOLD`). **3 Success** headers **9967–9969**, **1 Incomplete** (Gas and Supply `0040370068` / **9970** — multi-PO 58948/58890/58781, header + PDF, Select Receipts not posted), **0 Fail**, **6 HOLD** bills (all preflight-parse — invoice # from subject/filename: Nova/Erica Barrett 258145; Crosslink 27943/27944/27946; Telecom 15790; Insight 1809). Live GET after finish: Fastenal **9968** PO 58692 + 1 receipt line + PDF; EMJ **9969** PO 58913 + 1 receipt line + PDF; MSC **9967** Type 4 header + PDF (no PO). Note: **9967** vendor lookup matched live **1320-RMP INDUSTRIAL SUPPLY**, not MSC — left as-is (do not void). Outlook: Success → `Entered in AI`; Incomplete/HOLD → `AI HOLD`; Skipped noise → none. No follow-up flag. Mail.Send to Treyce: **email-sent** (exactly one, after the spreadsheet was locked), subject `AP dry run 10 — from 2026-08-18`. Cursor: `runs/daily-cursor.json` last_received **2026-08-19T16:59:12Z** (processed_count 488). Report: `runs/AP-run-2026-09-09-dry10.xlsx` (same-day 8/16 run already used `AP-run-2026-09-09.xlsx`). KIMCO UI was not opened. Later runs continue AFTER this cursor.
...
- CHECK STOP, statements, PODs, payment letters, dups, and not-a-bill are **noise**: walk past them, sheet-note as `Skipped`, do not stamp `AI HOLD`, do not consume the bill-attempt cap.
