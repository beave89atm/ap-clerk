...
**QUALITY V1.2** (Treyce 2026-09-10 notes on the 8/16 dry-10): `Success` means a **finished bill Treyce would not need to rework**, not a header create. See **[QUALITY.md](QUALITY.md)** for the fix-before-complete checklist, Outlook `Entered with issues`, and never-repeat regressions. This build does **not** re-arm the weekday daily 30. Next live touch continues AFTER the **2026-09-16** cursor (`2026-08-20T04:57:35Z`).
...
**Skip already-flagged (Kyle 2026-09-11).** Do not touch an email that already has Outlook `Entered in AI`, `AI HOLD`, `Entered with issues`, `AI Skipped 2`, leftover `AI Skipped`, or Graph `flag.flagStatus=flagged`. Leave it alone — no reprocess, no re-stamp. Already-flagged messages do **not** count toward the 10-email touch cap.
...
5. After enter, writes an Outlook category on `accountspayable@kannonmfg.com` only. **Success** (finished bill Treyce would not rework) gets `Entered in AI`. **Header+PDF entered but unfinished** (price-does-not-match, qty HOLD, Incomplete) gets **`Entered with issues`**. **Real bill with no header** gets red **`AI HOLD`**. **Noise** is sheet-noted as `Skipped` and gets **`AI Skipped 2`** (never `AI HOLD`, never `AI Skipped`). Never two process categories on the same message. Does **not** set `flag.flagStatus=flagged` and does **not** use `AP Matched`. Treyce created the Outlook category named exactly `AI Skipped 2` on accountspayable@. Old `AI Skipped` stamps remain and are walked past.
...
| **Skipped** | Mailbox noise. | `AI Skipped 2` |
...
| **HOLD** | Real bill gate failed before a trustworthy finished bill (price-does-not-match, missing PO, parse-error on a real invoice PDF, no receipts). Why names the gate. | `AI HOLD` |
| **Fail** | Already exists, vendor missing, or create HTTP error. | `AI HOLD` |
| **Skipped** | Mailbox noise among the ≤10 emails touched (not-a-bill, statement, CHECK STOP, payment, POD). Noted on the sheet. Consumes the email cap. | `AI Skipped 2` — never `AI HOLD` |

**Why** must name which gate failed. The **Notes** column is left empty for Treyce.

1. **Preflight parse** — the vendor PDF is the version of the truth (QUALITY.md **PDF-is-truth**). Subject/filename are hints. If a PDF is on disk, do not HOLD parse-error because the # was also tagged `subject` (Nova Alloys `258145`) or `filename` (Crosslink `27943` / `27944` / `27946`) — create the header, attach the PDF, continue finish gates. HOLD parse/no-pdf only when the PDF is truly missing or extract+OCR failed. **Why-must-be-true:** Why names this vendor, this invoice #, the # source (`pdf`/`subject`/`filename`), whether the PDF path existed, and the next action. Never append `(MSC/McQueary)` unless this bill is that vendor. `no-pdf-on-vm` is forbidden when `pdf_path` exists. If the PDF total cannot be verified: `HOLD parse-error (preflight-parse)`, no wrong-amount header. Vendor comes from the company on the PDF, never a From person’s name (`Erica Barrett`). Reject `Purchase_Order_*.pdf` and any attachment that is a PO, not an invoice. Regressions: Gas `0040323616` wrong amount; MSC/McQueary invoice # from filename when the PDF is missing; Legacy `Purchase_Order_58861.pdf`; Nova `258145`; Crosslink `27943`.
2. **PO** — never leave header Purchase Order blank when a PO is on the invoice **or** findable on live by vendor + part/WO. Multi-PO (3P / Rachel Bailey `142041`–`142044`): header PO blank, Invoice_Type 3 (receipt-type, not Misc 4), Select Receipts per PO; sheet lists each PO and selected receipts; unmatched PO is named on Why (never silent Success). Outlook is a bill category, never `AI Skipped 2`. If the printed PO is wrong/missing, search live POs by vendor + part numbers before Misc Type 4. Regressions: RMP `1470159`; Willbanks `209663` / `209664` “not misc, has PO”; 3P `142041`.
3. **Receipt** — before HOLD-no-receipts, second pass: slip # = invoice #, part, qty, PO line, then open receipts on that PO **verified by invoice qty and merchandise cost**. Prefer exact qty (35 vs 36 → 35). Never first-open / `second-open-on-po` Success when multiple open receipts differ. Match part/PO-WO lines (Modern Heat), not first leftover qty. Parse **every** merchandise line from the PDF (EMJ mill descriptions, not empty `lines:[]`). Select Receipts for **each** invoice line; unmatched lines are named on Why and never silent Success. A Gas billing PDF with N invoice numbers becomes **N bill rows** (after-tax Amount Due per invoice); the email still counts as **1** toward the 10-email cap. Already-entered vendor+# is HOLD `already-entered` before a misleading parse slogan. Regressions: Capital `26167`; Fastenal `TXFT499356`; Fastenal `TXFT4100079` / KIMCO **9968**; EMJ `Z250725432` / KIMCO **9969**; Gas `0040370068` pack / KIMCO **9970**; Insight `1809`.
4. **Finish** — `Success` requires header + (Select Receipts when PO) + PDF attached + **Additional Charge Fees posted when fees were parsed**. Header-only with blocked-405 attach, receipts not selected, or fees noted on the sheet but not posted = **Incomplete**, not Success, not Entered in AI. Live 2026-09-09: record GET/PUT/PDF attach and Select Receipts (record PUT of `lists.APInvoiceLine`) work API-only. Fees are a record PUT of `lists.APInvoiceAdditionalCharge` (`F-Fees & Surcharges`). No-PO bills can finish API-only (header + PDF). Do not open the KIMCO UI for attach.
...
If `KIMCO_LIVE_INSTANCE_URL` is unset, the live target uses `https://live.kimcoerp.com`. After Kyle said go, `enter --live` and `daily --live` authenticate and then create today's `API Agent - M/D/YY` batch plus invoice headers on live. Application `Mail.ReadWrite` is granted on the Kannon AP Clerk Entra app (admin consent 2026-08-28). After enter, the CLI PATCHes `Entered in AI` on Success, `Entered with issues` or `AI HOLD` on real-bill Incomplete/HOLD/Fail (V1.2), and exact **`AI Skipped 2`** on noise, on `accountspayable@kannonmfg.com` only. It does not set `flag.flagStatus`. If Graph cannot find/create `AI Skipped 2`, Why is `outlook-category-missing: AI Skipped 2` and the sheet row stays `Skipped`. QUALITY V1.1 does not run that live 30.

`enter --live --from-inbox --limit 10` touches at most **10** mailbox messages from `accountspayable@kannonmfg.com` (hard email cap; Kyle 2026-09-11). Already-flagged mail (`Entered in AI`, `AI HOLD`, `Entered with issues`, `AI Skipped 2`, leftover `AI Skipped`, or `flag.flagStatus=flagged`) is walked past and does not consume the cap. Noise among those 10 is sheet-noted as `Skipped`, stamped Outlook **`AI Skipped 2`**, and **does** consume the cap. Bill-attempt / walk-past-noise mode is suspended. It does not drain the mailbox.

## Weekday 5:00am daily run (hard-clamped to 10 emails)

Routine is paused. If daily is invoked: **hard-clamp to 10 emails** (Kyle 2026-09-11). Do not process 30 under this rule. `--limit 30` is clamped to 10 mailbox messages touched.

**Queue (FIFO toward today):**

- Start at received/invoice date **2026-07-28** inclusive (`America/Chicago`).
- Work **forward toward the most current**.
- **Skip already-flagged** (`Entered in AI`, `AI HOLD`, `Entered with issues`, `AI Skipped 2`, leftover `AI Skipped`, `flag.flagStatus=flagged`) without touching or consuming the cap.
...
**Weekday live-10 (2026-09-16 America/Chicago):** continued after the 9/15 cursor (`2026-08-19T20:12:03Z`, processed_count 480) on newest stacked quality tip `cursor/aqpc-plus5-batch711-25-307b` @ `e1d092d` (main still unmerged). Command: `python -m ap_clerk daily --live --limit 10 --as-of 2026-09-16`. Batch `API Agent - 9/16/26` id **715** (Status 0, not closed). Hard cap 10 honored (10 mailbox messages; 14 sheet rows because two Gas & Supply emails split). **0 Success**, **1 Incomplete**, **1 Fail**, **12 HOLD**, **0 Skipped**. Created headers **10047** (Priority 1 `18030910` freight fees blocked-400 Incomplete), **10048** (EMJ `Z250741432` — daily Type-4 Success reversed after live GET amount 0 / empty lines), **10049** (Metal Supermarkets `1091102` / PO 58919 qty HOLD), **10050** (O'Neal `14748440` / PO 58964 price-does-not-match $714.60 + @Shawn McKibben). Crosslink `27447`/`27448`/`27591` HOLD already-entered **9382**/**9384**/**9587**. McNichols `2559543` Fail vendor-missing (`billings@e.mcnichols.com`; PO 58935-MCNICHOLS exists). Gas split HOLDed (amounts not extracted; no invented totals). No invented Success. Live GET after run: 10047/10048 Type 4 amount 0 + PDF; 10049 Type 3 PO 58919 receipt 23185 + PDF; 10050 Type 3 PO 58964 0 lines + PDF; vendors 145 / 208 / 121 / 137. Outlook: EMJ patched `Entered in AI` → `Entered with issues`; Incomplete/header HOLD → `Entered with issues`; no-header HOLD/Fail → `AI HOLD`. Mail.Send to Treyce: **email-sent** (exactly one, from daily, subject `AP run 2026-09-16`). Did not re-email after the EMJ Success reversal. Cursor: `runs/daily-cursor.json` last_received **2026-08-20T04:57:35Z** (processed_count 494). Report: `runs/AP-run-2026-09-16.xlsx`. KIMCO UI was not opened. Next weekday continues AFTER this cursor.

**Weekday live-10 (2026-09-15 America/Chicago):** continued after the 9/14 afternoon cursor (`2026-08-19T00:04:36Z`, processed_count 466) on stacked tip `cursor/ai-skipped-2-437d` @ `5799d83`. Command: `python -m ap_clerk daily --live --limit 10`. Batch `API Agent - 9/15/26` id **711**. Hard cap 10 honored (10 mailbox messages; 14 sheet rows because two Legacy emails also carried receipt scans). **0 Success**, **0 Incomplete**, **0 Fail**, **13 HOLD**, **1 Skipped**. Created headers **9995** (Legacy `PS-INV103979` / PO 58807, qty-77 receipt HOLD) and **9996** (Legacy `PS-INV103980` / PO 58802, ambiguous open receipts). AQPC `10917`/`10918`/`10920`/`10921` HOLD `pdf-behind-link` (Intuit auth wall; not Skipped). Crosslink `27321`/`27319`/`27419` HOLD already-entered **9199**/**9193**/**9345**. **Greentree `Invoice from Greentree Packaging & Lumber` was a false statement skip** (NOTE-26) → `AI Skipped 2` / `no-pdf-on-vm` before the attached invoice PDF was read. No invented Success. Live GET after run: 9995/9996 vendor 292-Legacy, Type 3, PDF attached, batch 711 Status 0 (not closed). Outlook: bill HOLD with header → `Entered with issues`; no-header HOLD → `AI HOLD`; noise → `AI Skipped 2`. Mail.Send to Treyce: **email-sent** (exactly one, after the sheet was locked), subject `AP run 2026-09-15`. Did not re-email `AP-run-2026-09-14-live10.xlsx`. Cursor: `runs/daily-cursor.json` last_received **2026-08-19T20:12:03Z** (processed_count 480). Report: `runs/AP-run-2026-09-15.xlsx`. KIMCO UI was not opened. Next weekday continues AFTER this cursor.

**AQPC guest enter on batch 711 (2026-09-15 / 2026-09-16):** later plus-5 slices reused batch **711** (no Treyce Mail.Send). Current 40-row sheet: `runs/AP-run-2026-09-15-aqpc-batch711-40.xlsx` — **36 Success / 4 HOLD** (11003/10009, 10991/10013, 10925/10038, 9352/10045 BLM Fence not-Kannon-PO). 10956/**10021** finished with same-cost leftover **23517** (NOTE-27). Newest five: 10381/**10042**, 9502/**10043**, 9498/**10044**, 9352/**10045** HOLD, 9343/**10046**. Match already-entered by vendor + invoice # (AQPC 10938 ≠ JMOR 4779). See QUALITY.md.

**Supervised dry run 10 (2026-09-09 America/Chicago, from 2026-08-16):** Kyle asked for another 10 starting on date 8/16. FIFO from **2026-08-16** America/Chicago (did not restart at 7/28; skipped already-Success 9/8 finish-ups). Batch `API Agent - 9/9/26` id **703**. 10 real bills attempted (statement/payment/CHECK STOP/not-a-bill skips replaced). **5 Success** headers **9948–9952**, **0 Incomplete**, **0 Fail**, **5 HOLD** bills (Insight 1809 ×2 and Rob Brown/MSC 191471 preflight-parse — invoice # from filename; EMJ S813859432 price-does-not-match 17.1% / $199.32; Toyota Commercial Finance 3320056 PO 55483 not on live) plus skip HOLD rows. Live GET after finish: PO Success bills have receipt-linked `lists.APInvoiceLine` + PDF; Purvis no-PO **9950** has header + PDF. Outlook: Success → `Entered in AI`; HOLD → `AI HOLD`. No follow-up flag. Mail.Send to Treyce: **email-sent** (exactly one, after the spreadsheet was locked), subject `AP dry run 10 — from 2026-08-16`. Cursor: `runs/daily-cursor.json` last_received **2026-08-18T13:00:12Z** (processed_count 456). Report: `runs/AP-run-2026-09-09.xlsx`. KIMCO UI was not opened. Later runs continue AFTER this cursor.
...
**AQPC / Intuit guest click-through** (payment-request links on
`links.notification.intuit.com`). A human opens the link and gets the
invoice **without** signing in. Unauth GET first; auth/bot wall or
intermediate HTML opens Playwright as a guest and clicks View/Download
invoice. No Intuit login, no MFA, no saved session. CLI prints
present/absent only:

- `AP_CLERK_BROWSER_PDF` — `0`/`false`/`no` disables the browser escalate
- `AP_CLERK_BROWSER_PDF_TIMEOUT` — seconds (default 45)
- `AP_CLERK_INTUIT_STORAGE_STATE` / `AP_CLERK_INTUIT_COOKIE_JAR` — optional
  for other vendor portals later; **not required for AQPC** and not a
  standing fix if the guest browser fails.
...
Fees and surcharges (shop supplies, packaging recovery, fuel/energy surcharge, freight, shipping & handling, admin/account/check fees, garment protection, rental, and similar add-ons) are **not** Purchase Price Variance. They go to Additional Charge **Fees and surcharges** / **F-Fees & Surcharges** on the KIMCO bill (record PUT), and they are recorded in the Excel **Fees and surcharges** column. The sheet column alone is **not** a post (Fastenal `TXFT4100079` / 9968). If a parsed fee amount cannot be posted, Result is Incomplete / Entered with issues, never Success. Never post those as PPV.
...
6. Categories: Success = `Entered in AI`; bill Incomplete/HOLD/Fail = `AI HOLD`. Noise / `Skipped` = no Outlook category. Never set `flag.flagStatus=flagged`.
...
**Flag in Outlook:** `Yes` when a process category is applied (Success, Incomplete, bill HOLD, Fail, or Skipped noise). **Flag status** is `entered-in-ai` (Success only) / `entered-with-issues` (header+PDF but unfinished) / `ai-hold` (real bill, no header / Fail) / `ai-skipped` (noise) / other skip reasons (`no-message-id`, `graph-denied`, `skipped-not-success`). Incomplete and price/qty HOLD with a header must not say `entered-in-ai`. Noise must not say `ai-hold`. **Notes** is left empty for Treyce. Why names the gate that failed (or the noise class + subject) so she is not hunting.
...
- **Success** (finished bill Treyce would not rework): PATCH categories to include `Entered in AI` and **remove** the other process markers.
...
- **Noise / Skipped:** PATCH exact string **`AI Skipped 2`** and remove the other process markers. Sheet Result stays `Skipped` with Why. Never `AI HOLD` for noise. Do **not** leave noise uncategorized. If Graph cannot find/create the master category, Why is `outlook-category-missing: AI Skipped 2`. Historical `AI Skipped` stamps are not rewritten.

PATCH body is `{"categories":[<existing except AP Matched and the other process markers>, "<Entered in AI|Entered with issues|AI HOLD|AI Skipped 2>"]}`. It does **not** set `flag.flagStatus`. It does **not** add `AP Matched`. Never two process categories on the same message.
