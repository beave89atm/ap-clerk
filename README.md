...
`--match-inbox` on `enter` attaches Graph message ids onto fixture invoices, then flags **after** a Success header create — never after download alone. `--mailbox` must be `accountspayable@kannonmfg.com`; any other mailbox (including `POD@kannonmfg.com`) is rejected.

**NOTE-48 POD mailbox** (Kyle 2026-09-22): `POD@kannonmfg.com` is a shared mailbox for **proof-of-delivery** email only. Same `MICROSOFT_GRAPH_*` client-credentials as the AP mailbox. Read-only (`GET` list / message / attachment names). Do **not** enter invoices from POD@. Do **not** Mail.Send from POD@.

```bash
python3 -m ap_clerk pod
```
...
The only **invoice** mailbox this CLI will touch is `accountspayable@kannonmfg.com`. Mail without a process category (`Entered in AI`, `Entered with issues`, `AI HOLD`, `AI Skipped 2`, leftover `AI Skipped`) is the work queue. `POD@kannonmfg.com` is POD intake only (NOTE-48) — Graph GET, never enter / flag / sendMail.
