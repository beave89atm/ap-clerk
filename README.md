...
1. **Preflight parse** — invoice #, date, amount, and PO are taken from the **vendor PDF text**, not email subject/filename alone. If the PDF is on disk, do not HOLD parse-error just because the # was also tagged from the subject (Nova Alloys `258145` / 8/18) or the filename (Crosslink `27943` / `27944` / `27946`). Filename/subject matching the PDF is OK; confirm the # from PDF text when present. `no-pdf-on-vm` is forbidden when `pdf_path` exists. If the PDF total cannot be verified: `HOLD parse-error (preflight-parse)`, no wrong-amount header. Vendor comes from the company on the PDF/subject, never a From person’s name (`Erica Barrett`). Reject `Purchase_Order_*.pdf` and any attachment that is a PO, not an invoice. Regressions: Gas `0040323616` wrong amount; MSC/McQueary invoice # from filename; Legacy `Purchase_Order_58861.pdf`; Nova `258145`; Crosslink `27943`.
